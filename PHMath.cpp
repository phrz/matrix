...
namespace PH {
	...
	MathNumber dotProduct(const Matrix& A, const Matrix& B) {
		...
		if (A.columns() != B.columns() || A.rows() != B.rows()) {
			throw new std::invalid_argument("dotProduct: incompatible matrix dimensions (must be same)");
		}
		
		MathNumber sum = 0.0;
		A.mapElements([sum](MathNumber& aElement, Index row, Index column) {
			sum += aElement * B(row, column);
		});
	...
	MathNumber dotProduct(const Vector& v1, const Vector& v2) {
		if (v1.size() != v2.size()) {
			throw new std::invalid_argument("dotProduct: incompatible vector sizes (must be same)");
		}
	...
	Vector linSpace(MathNumber a, MathNumber b, Index n) {
		if (n<2) {
	...
	Vector logSpace(MathNumber a, MathNumber b, Index n) {
		if (n<2) {
	...
	Vector randomVectorOfSize(Index n) {
		...
		Vector result(n);
		result.mapElements([uniform, generator](MathNumber& element, Index i){
			element = double(uniform(generator)) / RAND_MAX;
		});
	...
	Vector matrixVectorProduct(const Matrix& A, const MathVector& v) {
		...
		A.mapElements([result](MathNumber& aElement, Index row, Index column){
			result += aElement * v[column];
		});
